...
export function InfoCheckbox({ icon: Icon, iconColor, text, checkboxLabel }) {
    ...
    return (
        <div className="border border-gray-200 rounded-xl p-5 hover:border-blue-400 hover:shadow-md transition-all duration-200 bg-white">
            <div className="flex items-start gap-4">
                <div className={`p-2 rounded-lg ${iconColor === 'amber' ? 'bg-amber-50' : iconColor === 'blue' ? 'bg-blue-50' : iconColor === 'red' ? 'bg-red-50' : 'bg-green-50'}`}>
                    <Icon className={`w-5 h-5 ${iconColor === 'amber' ? 'text-amber-600' : iconColor === 'blue' ? 'text-blue-600' : iconColor === 'red' ? 'text-red-600' : 'text-green-600'}`} />
                </div>
