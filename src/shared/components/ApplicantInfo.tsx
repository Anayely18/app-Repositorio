...
export function ApplicantInfo({
  ...
  return (
    <div className={`grid grid-cols-1 gap-6 ${hidePhone ? "lg:grid-cols-3" : "lg:grid-cols-4"}`}>
      ...
      {/* teléfono (solo si NO está oculto) */}
      {!hidePhone && (
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="font-semibold text-xs text-gray-900">{applicant?.phone}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="font-semibold text-xs text-gray-900">{formatDate(createdAt)}</p>
        </div>
      </div>
    </div>
  );
